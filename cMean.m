...
critFun = 0;
for i = 1 : size(Y1, 1)
    critFun = critFun + sum((Y1(i, :) - M1).^2);
end
for i = 1 : size(Y2, 1)
    critFun = critFun + sum((Y2(i, :) - M2).^2);
end
for i = 1 : size(Y3, 1)
    critFun = critFun + sum((Y3(i, :) - M3).^2);
end
for i = 1 : size(Y4, 1)
    critFun = critFun + sum((Y4(i, :) - M4).^2);
end
for i = 1 : size(Y5, 1)
    critFun = critFun + sum((Y5(i, :) - M5).^2);
end

critFun = critFun / (num_points * 4)  + log(num_classes);
